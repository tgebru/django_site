...
for make_ind,make in enumerate(makes):
  ...
  sql_s='select distinct(model) from geocars.synsets where make="%s"'%make
  cursor.execute(sql_s)
  models=cursor.fetchall()
  make_model_dict[make]=[m[0].replace(' ','-') for m in models]
#print(make_model_dict)
pickle.dump(make_model_dict,open(os.path.join(file_dir,'make_model_dict-new.p'),'wb'),protocol=2)
